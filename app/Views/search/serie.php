<div id="main">
    <div class="inner">
        ...
        <?php if (key_exists('Search', $results)): ?>
            <section class="tiles">
                <?php foreach ($results['Search'] as $serie) : ?>
                    <article class="">
                        <span class="image">
                            <img src="<?= esc($serie['Poster']) ?>" alt="" />
                        </span>
                        <a href="<?= esc(base_url() . 'serie/details/' . aesEncrypt($serie['imdbID']) . '.html') ?>" target="_self">
                            <h2><?= esc($serie['Title']) ?></h2>
                            <div class="content">
                                <p><?= esc($serie['Year']) ?></p>
                            </div>
                        </a>
            ...
            <header id="header" class="pages">
                <?php if ($page <= 1): ?>
                    <span></span>
                <?php else: ?>
                    <a href="<?= esc(base_url() . 'serie/search/' . ($page - 1) . '/' . $search) ?>" target="_self">
                ...
                <?php endif; ?>
                <a href="<?= esc(base_url() . 'serie/search/' . $page . '/' . $search . '#') ?>" target="_self">
